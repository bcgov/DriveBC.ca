...
class TestWildfireModel(BaseTest):
    def setUp(self):
        ...
        # Normal feed
        wildfire_feed_data = open(
            str(Path(__file__).parent) +
            "/test_data/wildfire_feed_list.json"
        )
        self.mock_wildfire_feed_result = json.load(wildfire_feed_data)

        # Manually updated feed with one less ferry vessel
        wildfire_feed_data = open(
            str(Path(__file__).parent) +
            "/test_data/wildfire_feed_list_updated.json"
        )
        self.mock_updated_wildfire_feed_result = json.load(wildfire_feed_data)
    ...
    def test_populate_and_update_wildfires(self, mock_requests_get):
        mock_requests_get.side_effect = [
            MockResponse(self.mock_wildfire_feed_result, status_code=200),
            MockResponse(self.mock_updated_wildfire_feed_result, status_code=200),
        ]
