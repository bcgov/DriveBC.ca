...
import HighwayFilter from '../Components/shared/HighwayFilter';
...
export default function CamerasListPage() {
  ...
  const [showHighwayFilter, setShowHighwayFilter] = useState(true);

  // Staged filter selections, only committed to the filter context on Apply
  ...
  const toggleFiltersOverlay = () => {
    if (!showFilters) {
      // Stage the currently applied filters when opening the overlay
  ...
  const applyFilters = () => {
    setFilterContext({
      ...filterContext,
      areaFilter: pendingAreaFilter,
      highwayFilterKey: pendingHighwayFilterKey
    });
  ...
  // Used outside the overlay: clears the applied filters as well as the draft,
  // since resetting the draft alone has no visible effect when the overlay is closed
    ...
    <React.Fragment>
      ...
      <div className={`overlay filters-overlay ${showFilters ? 'open' : ''}`}>
        ...
        <div className="filters-component">
          <div
            className={'filters-overlay__subheader bold' + (showAreaFilter ? ' open' : '')}
            role="button"
            tabIndex={0}
            aria-expanded={showAreaFilter}
            onClick={() => setShowAreaFilter(!showAreaFilter)}
            onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setShowAreaFilter(!showAreaFilter); } }}>
          ...
          <div className={'filters-overlay__collapsible' + (showAreaFilter ? ' open' : '')}>
            <div className="filters-overlay__collapsible-inner">
              <AreaFilter
                handleAreaFiltersClose={() => {}}
                objects={displayedCameras}
          ...
          <div
            className={'filters-overlay__subheader bold' + (showHighwayFilter ? ' open' : '')}
            role="button"
            tabIndex={0}
            aria-expanded={showHighwayFilter}
            onClick={() => setShowHighwayFilter(!showHighwayFilter)}
            onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setShowHighwayFilter(!showHighwayFilter); } }}>
          ...
          <div className={'filters-overlay__collapsible' + (showHighwayFilter ? ' open' : '')}>
            <div className="filters-overlay__collapsible-inner">
              {processedCameras &&
                <HighwayFilter
        ...
        <div className="apply-actions">
          <Button
            variant="outline-primary"
            className="apply-actions__button"
