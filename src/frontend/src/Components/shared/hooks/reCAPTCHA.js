...
// Env Variables
import { RECAPTCHA_CLIENT_ID } from "../../../env.js";

export const useRecaptchaVerification = () => {
  useEffect(() => {
    const script = document.createElement('script');
    script.src = `https://www.google.com/recaptcha/api.js?render=${RECAPTCHA_CLIENT_ID}`;
    script.async = true;
    script.defer = true;
    ...
    if (typeof window !== 'undefined' && window.grecaptcha) {
      return new Promise((resolve, reject) => {
        window.grecaptcha.ready(() => {
          window.grecaptcha.execute(RECAPTCHA_CLIENT_ID, { action }).then(resolve).catch(reject);
        });
      });
    }
