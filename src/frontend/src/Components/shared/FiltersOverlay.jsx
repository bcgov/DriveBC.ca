...
export default function FiltersOverlay(props) {
  ...
  const applyFilters = () => {
    ...
    if (showDelayTypes) {
      DELAY_TYPES.forEach((delayType) => {
        if (!!pendingDelayTypes[delayType.key] !== !!mapContext.visible_layers[delayType.key]) {
      ...
      <div className="filters-component">
        ...
        {showDelayTypes &&
          <FiltersOverlaySection
            ...
            icon={<FontAwesomeIcon icon={faTrafficCone} />}
            title={`Delay types (${selectedDelayTypeCount})`}>
            <DelayTypeFilter
