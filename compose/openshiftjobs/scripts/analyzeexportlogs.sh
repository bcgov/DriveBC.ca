...
if [ ${#zipped_files[@]} -gt 0 ]; then
    ...
    zcat "${zipped_files[@]}" | grep -v '^-\s' | sed 's/] "GET/] "" "GET/g' | sed 's/] "POST/] "" "POST/g' | goaccess - -o "$goaccess_report_name" --log-format='~h{, } %e %^[%x] "%v" "%r" %s %b "%R" "%u" %C "%M" %T' --datetime-format='%d/%b/%Y:%H:%M:%S %z' --ignore-panel=REMOTE_USER --ignore-panel=ASN --tz=America/Vancouver --jobs=2 --geoip-database=$mmdb_file
